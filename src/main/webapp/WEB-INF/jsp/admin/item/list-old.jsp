...
<script type="text/javascript">
	...
	$(function() {
		...
		var table = $("#myTable").DataTable(
			{
				...
				"ajaxSource" : "${pageContext.request.contextPath}/admin/item/listItemByPage",//这个是请求的地址
				...
				"columns" : [
						...
						},
						{
							"data" : "status","render" : function(data, type, row) {
								var status;
								data=="1"? status = "Enable":status="Disabel";
								return status;
							}
						},
						{
							...
							"render" : function(data, type, row) {
								var button;
								button = '<a href="${pageContext.request.contextPath}/admin/item/update/'+row.itemId+'" class="update btn btn-xs btn-warning glyphicon glyphicon-edit" data-trigger="hover" data-toggle="tooltip" data-placement="top" title="Edit"></a>&nbsp'
									...
									+ '<button itemId="'+row.itemId+'" class="remove btn btn-xs btn-danger glyphicon glyphicon-trash" data-trigger="hover" data-toggle="tooltip" data-placement="top" title="Delete"></button>'
								return button;
							}
						}
				],
			...
			type:'red',
			 buttons: {
				 Delete:{
					 ...
					 btnClass:'btn',
					 action:function(){
						 $.get("${pageContext.request.contextPath}/admin/item/delete/"+id,function(flag){
								if(flag){
	   ...
	   table.columns().eq( 0 ).each( function ( colIdx ) {
		...
		$("div.new").html('<a href="${pageContext.request.contextPath}/admin/item/add" id="new" class="dt-button" style="float:left">New</a>');
