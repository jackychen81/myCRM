	...
	$(function() {
		...
		var table = $("#myTable").DataTable(
			{
				...
				"ajaxSource" : "${pageContext.request.contextPath}/admin/user/listUserByPage",//这个是请求的地址
				...
				"columns" : [
						...
						},
						{
							...
							"render" : function(data, type, row) {
								var button;
								button = '<a href="${pageContext.request.contextPath}/admin/user/update/'+row.id+'" class="update btn btn-xs btn-warning glyphicon glyphicon-edit" data-trigger="hover" data-toggle="tooltip" data-placement="top" title="Edit"></a>&nbsp'
									+'<button userId="'+row.id+'" class="setRole btn btn-xs btn-default glyphicon glyphicon-edit" data-trigger="hover" data-toggle="tooltip" data-target="#myModal" data-placement="top" title="Set Role"></button>&nbsp'
									+ '<button userId="'+row.id+'" class="remove btn btn-xs btn-danger glyphicon glyphicon-trash" data-trigger="hover" data-toggle="tooltip" data-placement="top" title="Delete"></button>'
								return button;
							}
						}
				],
		...
		$('#myTable').on('click','.details-control',function(e){
	        ...
	        if ( row.child.isShown() ) {
	            ...
	            // Open this row
	            row.child(format(row.data().card)).show();
		...
		function format(d){
			...
			if(d!=null && d.length>0){
				for(var i =0; i<d.length;i++){
					  tr = tr+'<tr>'+
					  			'<td>'+d[i].cardCategory.name+'</td>'+
								'<td>'+d[i].description+'</td>'+
								'<td>'+d[i].amount+'</td>'+
								'<td>'+d[i].plusAmount+'</td>'+
								'<td>'+d[i].remaining+'</td>'+
